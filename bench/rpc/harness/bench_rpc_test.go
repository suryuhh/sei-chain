...
func TestNode1(t *testing.T) {
	...
	if serveSrv != nil {
		...
		// The steady span's wall-clock bounds (blocks[B/10].done to the last block's done), for the client's
		// account of the requests due, answered and outstanding in it.
		app.mu.Lock()
		done := slices_clone(app.blocks)
		app.mu.Unlock()
		if sf := max(1, len(done)/10); sf < len(done) {
			fmt.Fprintf(&bh, "span %d %d\n", done[sf].done.UnixNano(), done[len(done)-1].done.UnixNano())
		}
...
// ---- RPC receipt-query load: the EVM-only eth_getTransactionReceipt path, in process.
...
func rpcqRun(t *testing.T, giga p2p.GigaRouter, rdbAny any, bs rpcqBlockStore, txs [][]byte, lastHeight int64) {
	...
	// Block hashes for the lean mode, read once per height.
	...
	modes := strings.Split(node1EnvStr("NODE1_RPCQ_MODES", "lean,full,block,lean,full,block"), ",")
	for _, mode := range modes {
		...
		cpu := rpcqCPU() - c0
		fmt.Printf("RPCQ mode=%s workers=%d n=%d blocks=%d wall_s=%.3f qps=%.0f cpu_us_per_q=%.1f bytes_per_q=%d\n",
			mode, workers, n, len(blockHash), wall, float64(n)/wall, cpu*1e6/float64(n), bytesOut.Load()/int64(n))
	}
}
...
func (b rpcqBackend) EvmBaseFee() (*big.Int, error) {
	return b.app.(interface{ EvmBaseFee() *big.Int }).EvmBaseFee(), nil
}
...
func (b rpcqBackend) EvmChainConfig() (*params.ChainConfig, error) {
	return b.app.(interface{ EvmChainConfig() *params.ChainConfig }).EvmChainConfig(), nil
}
func (b rpcqBackend) EvmChainID() uint64 {
	return b.app.(interface{ EvmChainID() uint64 }).EvmChainID()
}
func (b rpcqBackend) EvmGasLimit() (uint64, error) {
	return b.app.(interface{ EvmGasLimit() uint64 }).EvmGasLimit(), nil
}
func (b rpcqBackend) EvmMinGasPrice() (*big.Int, error) {
	return b.app.(interface{ EvmMinGasPrice() *big.Int }).EvmMinGasPrice(), nil
}
...
func (b rpcqBackend) EvmTransactionCount(a common.Address) uint64 {
	return b.app.(interface{ EvmNonce(common.Address) uint64 }).EvmNonce(a)
}

// BlockTx serves a tree whose EVM-only RPC reads one transaction of a block, from the router's
// BlockTxByNumber; a tree without that read never calls it.
func (b rpcqBackend) BlockTx(ctx context.Context, req *coretypes.RequestBlockInfo, index int) (*coretypes.ResultBlock, error) {
	n, err := b.height(req)
	if err != nil {
		return nil, err
	}
	rpcqLiveBlockReads.Add(1)
	g, ok := b.giga.(interface {
		BlockTxByNumber(context.Context, atypes.GlobalBlockNumber, int) (*coretypes.ResultBlock, error)
	})
	if !ok {
		panic("router has no BlockTxByNumber")
	}
	return g.BlockTxByNumber(ctx, n, index)
}
...
func TestRPCQClient(t *testing.T) {
	...
	reqs := make(chan request, len(hashes)+16)
	var answered, wrongBlock, httpErrs, retries, requested atomic.Int64
	every := node1EnvInt("NODE1_RPCQ_EVERY", 1)
	// NODE1_RPCQ_METHOD names what one requested transaction costs the node:
	//   receipt  eth_getTransactionReceipt;
	//   latest   eth_getBlockByNumber("latest", false), which wallets send before signing;
	//   wallet   the reads a viem wallet makes for one transaction it sends and awaits, in its order:
	//            the latest block, the sender's transaction count, the transaction by hash, the receipt;
	//   call     eth_call of creation code returning (NUMBER, token.balanceOf(sender)) at "latest", with a
	//            refusal returned to the caller as a JSON-RPC error.
	method := node1EnvStr("NODE1_RPCQ_METHOD", "receipt")
	accounts := node1EnvInt("NODE1_ACCOUNTS", 4000)
	senders := make([]common.Address, accounts)
	parallelFor(accounts, func(i int) { senders[i] = crypto.PubkeyToAddress(node1Key(uint64(i)).PublicKey) })
	answerSum := make([][32]byte, len(hashes))
	answerHash := make([]string, len(hashes))
	txSum := make([][32]byte, len(hashes))
	txHash := make([]string, len(hashes))
	var hashBad, txBad, countBad, latestChecked, latestBad, refused, callChecked, callBad atomic.Int64
	var refusedMsg atomic.Pointer[string]
	lat := make([]float64, len(hashes))
	doneAt := make([]int64, len(hashes))
	dueAt := make([]int64, len(hashes))
	for i := range lat {
		lat[i] = -1
	}
	start := time.Now()
	...
	// rpc sends body until it is answered with a non-null result, the node returns an error that is not a
	// transport failure (returned when refusals reach the caller), or the client's time runs out.
	rpc := func(body string, errorsReachCaller bool) (json.RawMessage, string, bool) {
		for time.Now().Before(end) {
			...
			if out.Error != nil {
				if errorsReachCaller {
					return nil, out.Error.Message, true
				}
				httpErrs.Add(1)
				time.Sleep(200 * time.Millisecond)
				continue
	...
	blockOf := func(i int) int64 { return first + int64(i/perBlock) }
	receipt := func(i int) bool {
		res, _, ok := rpc(fmt.Sprintf(`{"jsonrpc":"2.0","id":1,"method":"eth_getTransactionReceipt","params":["%s"]}`, hashes[i]), false)
		if !ok {
			return false
		}
		var head struct {
		...
		if json.Unmarshal(res, &head) != nil || int64(head.BlockNumber) != blockOf(i) {
			wrongBlock.Add(1)
		}
		...
		hs := rpcqBlockHashRE.FindAllSubmatch(res, -1)
		if len(hs) == 0 {
			hashBad.Add(1)
		} else {
			answerHash[i] = string(hs[0][1])
			for _, m := range hs[1:] {
				if string(m[1]) != answerHash[i] {
					hashBad.Add(1)
				}
			}
		}
		answerSum[i] = sha256.Sum256(rpcqBlockHashRE.ReplaceAll(res, []byte(`"blockHash":""`)))
		return true
	}
	latest := func(i int) bool {
		res, _, ok := rpc(`{"jsonrpc":"2.0","id":1,"method":"eth_getBlockByNumber","params":["latest",false]}`, false)
		if !ok {
			return false
		}
		if (i/every)%25 == 0 {
			// Sampled byte check, made in the client: the answer's hash list is its block's transactions, in order.
			latestChecked.Add(1)
			var blk struct {
			...
			if json.Unmarshal(res, &blk) != nil {
				latestBad.Add(1)
			} else if lo := (int64(blk.Number) - first) * int64(perBlock); lo < 0 || lo+int64(perBlock) > int64(len(hashes)) || len(blk.Transactions) != perBlock {
				latestBad.Add(1)
			} else {
				for k, h := range blk.Transactions {
					if !strings.EqualFold(h, hashes[lo+int64(k)]) {
						latestBad.Add(1)
						break
					}
				}
	...
	count := func(i int) bool {
		res, _, ok := rpc(fmt.Sprintf(`{"jsonrpc":"2.0","id":1,"method":"eth_getTransactionCount","params":["%s","latest"]}`, senders[i%accounts].Hex()), false)
		if !ok {
			return false
		}
		var n hexutil.Uint64
		// Transaction i is the sender's (i/accounts)-th and is already in a reported block.
		if json.Unmarshal(res, &n) != nil || uint64(n) < uint64(i/accounts)+1 {
			countBad.Add(1)
		}
		return true
	}
	byHash := func(i int) bool {
		res, _, ok := rpc(fmt.Sprintf(`{"jsonrpc":"2.0","id":1,"method":"eth_getTransactionByHash","params":["%s"]}`, hashes[i]), false)
		if !ok {
			return false
		}
		var tx struct {
		...
		if json.Unmarshal(res, &tx) != nil || !strings.EqualFold(tx.Hash, hashes[i]) || int64(tx.BlockNumber) != blockOf(i) ||
			int(tx.TransactionIndex) != i%perBlock || tx.From != senders[i%accounts] {
			txBad.Add(1)
		}
		txHash[i] = tx.BlockHash
		txSum[i] = sha256.Sum256(rpcqBlockHashRE.ReplaceAll(res, []byte(`"blockHash":""`)))
		return true
	}
	ethCall := func(i int) bool {
		// Accounts 1..perBlock-1 pay one token in the blocks first, first+2, ... and are repaid in the others.
		acct := 1 + i%(perBlock-1)
		code := common.FromHex("0x6370a0823160e01b600052" + "73" + hex.EncodeToString(senders[acct][:]) +
			"60045260206040602460007300000000000000000000000000000000000000005afa504360205260406020f3")
		copy(code[44:64], node1ERC20[:])
		res, msg, ok := rpc(fmt.Sprintf(`{"jsonrpc":"2.0","id":1,"method":"eth_call","params":[{"data":"%s"},"latest"]}`, hexutil.Encode(code)), true)
		if !ok {
			return false
		}
		if msg != "" {
			refused.Add(1)
			if refusedMsg.Load() == nil {
				refusedMsg.Store(&msg)
			}
			return true
		}
		callChecked.Add(1)
		var out hexutil.Bytes
		if json.Unmarshal(res, &out) != nil || len(out) != 64 {
			callBad.Add(1)
			return true
		}
		n := new(big.Int).SetBytes(out[:32]).Int64()
		want := new(big.Int).Set(node1TokenBalance)
		if n >= first && (n-first)%2 == 0 {
			want.Sub(want, node1TransferValue)
		}
		if new(big.Int).SetBytes(out[32:]).Cmp(want) != 0 {
			callBad.Add(1)
		}
		return true
	}
	flow := map[string][]func(int) bool{
		"receipt": {receipt},
		"latest":  {latest},
		"wallet":  {latest, count, byHash, receipt},
		"call":    {ethCall},
	}[method]
	require.NotNil(t, flow, "NODE1_RPCQ_METHOD %q", method)
	var wg sync.WaitGroup
	for w := 0; w < workers; w++ {
		wg.Add(1)
		go func() {
	...
	// Follow the chain head; each newly reported block's requests become due after delay. With
	// NODE1_RPCQ_SPREAD=1 a block's requests are spread evenly over the time since the previous head
	// advance, as independent users' would be, rather than all due at once.
	spread := os.Getenv("NODE1_RPCQ_SPREAD") == "1"
	...
	for time.Now().Before(end) && int(next-first)*perBlock < len(hashes) {
		...
		if err == nil && json.Unmarshal(b, &out) == nil {
			...
			if spread && !lastAdvance.IsZero() && next <= int64(out.Result) {
				period = now.Sub(lastAdvance) / time.Duration(int64(out.Result)-next+1)
			}
			if next <= int64(out.Result) {
				lastAdvance = now
			}
			for k := int64(0); next <= int64(out.Result) && int(next-first)*perBlock < len(hashes); next, k = next+1, k+1 {
				lo := int(next-first) * perBlock
				for i := lo; i < min(lo+perBlock, len(hashes)); i++ {
					if i%every == 0 {
						off := period * time.Duration(k)
						if period > 0 {
							off += period * time.Duration(i-lo) / time.Duration(perBlock)
						}
						requested.Add(1)
						reqs <- request{i: i, due: now.Add(delay + off)}
					}
				}
			}
		}
	...
	for deadline := time.Now().Add(2 * time.Minute); time.Now().Before(deadline); time.Sleep(100 * time.Millisecond) {
		if raw, err := os.ReadFile(hf + ".blocks"); err == nil {
			for _, line := range strings.Split(strings.TrimSpace(string(raw)), "\n") {
				var h int64
				var hexHash string
				if _, err := fmt.Sscanf(line, "span %d %d", &spanStart, &spanEnd); err == nil {
					continue
				}
				if _, err := fmt.Sscanf(line, "%d %s", &h, &hexHash); err == nil {
					nodeHash[h] = strings.ToLower(hexHash)
				}
			}
			break
		}
	}
	var hashChecked, txChecked int64
	for i := range answerHash {
		...
		if method == "wallet" && doneAt[i] != 0 {
			txChecked++
			if nodeHash[first+int64(i/perBlock)] != strings.ToLower(txHash[i]) {
				txBad.Add(1)
			}
		}
	}
	fmt.Printf("RPCQCLIENT node_heights=%d hash_checked=%d hash_mismatch=%d\n", len(nodeHash), hashChecked, hashBad.Load())
	// Requests due in the node's steady span, those answered within it, and those due in it and still
	// outstanding at its end.
	var spanDue, spanDone, spanOutstanding int64
	for i := range dueAt {
		if i%every != 0 || dueAt[i] == 0 || dueAt[i] < spanStart || dueAt[i] > spanEnd {
			continue
		}
		spanDue++
		if doneAt[i] != 0 && doneAt[i] <= spanEnd {
			spanDone++
		} else {
			spanOutstanding++
		}
	}
	fmt.Printf("RPCQSPAN method=%s span_s=%.6f due=%d answered_in_span=%d outstanding_at_end=%d\n",
		method, float64(spanEnd-spanStart)/1e9, spanDue, spanDone, spanOutstanding)
	if method == "wallet" {
		td := sha256.New()
		for i := range txSum {
			if i%10 == 0 {
		...
		fmt.Printf("RPCQWALLET tx_checked=%d tx_mismatch=%d count_mismatch=%d tx_digest10=%x\n", txChecked, txBad.Load(), countBad.Load(), td.Sum(nil))
	}
	if method == "latest" || method == "wallet" {
		fmt.Printf("RPCQLATEST checked=%d mismatch=%d\n", latestChecked.Load(), latestBad.Load())
	}
	if method == "call" {
		msg := ""
		if m := refusedMsg.Load(); m != nil {
			msg = *m
		}
		fmt.Printf("RPCQCALL checked=%d state_mismatch=%d refused=%d first_refusal=%q\n", callChecked.Load(), callBad.Load(), refused.Load(), msg)
	}
