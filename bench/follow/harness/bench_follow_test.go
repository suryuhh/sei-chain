...
//
// Benchmark harness for sei-tendermint/internal/p2p, built only with -tags benchharness. It is an external test
// package (p2p_test)
...
func (a *node1App) FinalizeBlock(ctx context.Context, req *abci.RequestFinalizeBlock) (*abci.ResponseFinalizeBlock, error) {
	...
	node1ValHashes.Store(req.Header.Height, bytes.Clone(resp.AppHash))
...
// mempool is at capacity, so every later block starts with that backlog already handed over.
...
func TestNode1(t *testing.T) {
	...
	followers := startNode1Followers(t, cfg, wl, genDoc, validators, addrs)
	...
	err = scope.Run(ctx, func(ctx context.Context, s scope.Scope) error {
		...
		for _, run := range followers.runs {
			s.SpawnBgNamed("follower", func() error { return utils.IgnoreCancel(run(ctx)) })
		}
	...
	if serveSrv != nil {
		...
		// file, at most NODE1_RPCQ_HOLD_S.
...
func rpcqRun(t *testing.T, giga p2p.GigaRouter, rdbAny any, bs rpcqBlockStore, txs [][]byte, lastHeight int64) {
	...
	// Block hashes for the lean variant, read once per height.
	...
	variants := strings.Split(node1EnvStr("NODE1_RPCQ_VARIANTS", "lean,full,block,lean,full,block"), ",")
	for _, variant := range variants {
		...
		cpu := rpcqCPU() - c0
		fmt.Printf("RPCQ variant=%s workers=%d n=%d blocks=%d wall_s=%.3f qps=%.0f cpu_us_per_q=%.1f bytes_per_q=%d\n",
			variant, workers, n, len(blockHash), wall, float64(n)/wall, cpu*1e6/float64(n), bytesOut.Load()/int64(n))
	}
}
...
func (b rpcqBackend) EvmCall(context.Context, *ethcore.Message) (*ethcore.ExecutionResult, error) {
	panic("rpcq: EvmCall")
}
...
// would: a null result sleeps NODE1_RPCQ_SLEEP_MS and retries.
...
// NODE1_RPCQ_LIVE=serve mode: each transaction's receipt is requested once, NODE1_RPCQ_DELAY_MS after
...
func TestRPCQClient(t *testing.T) {
	...
	tasks := make(chan task, len(hashes)+16)
	var answered, wrongBlock, httpErrs, retries, requested atomic.Int64
	every := node1EnvInt("NODE1_RPCQ_EVERY", 1)
	// NODE1_RPCQ_DUMP names a file that receives every NODE1_RPCQ_DUMP_EVERY-th answer (diagnostic only).
		...
		if err == nil && json.Unmarshal(b, &out) == nil {
			...
			for ; next <= int64(out.Result) && int(next-first)*perBlock < len(hashes); next++ {
				...
				for i := lo; i < min(lo+perBlock, len(hashes)); i++ {
					if i%every == 0 {
						requested.Add(1)
						tasks <- task{i: i, due: now.Add(delay)}
					}
				}
			}
		}
...
// ---- followers: fullnodes that pull the validator's finalized blocks over its Giga port.

// node1MaxTPS paces the producer when NODE1_MAX_TPS is set, so the chain finalizes a stated number of blocks per second.
func node1MaxTPS() utils.Option[uint64] {
	if v := node1EnvInt("NODE1_MAX_TPS", 0); v > 0 {
		return utils.Some(uint64(v))
	}
	return utils.None[uint64]()
}

// node1FollowApp is a fullnode's real evmOnlyApplication that records each height's AppHash and completion time.
type node1FollowApp struct {
	abci.Application
	canon *node1App
	mu    sync.Mutex
	hash  map[int64][]byte
...
func (a *node1FollowApp) PrepareBlock(ctx context.Context, req *abci.RequestFinalizeBlock) error {
	p, ok := a.Application.(interface {
		PrepareBlock(context.Context, *abci.RequestFinalizeBlock) error
	})
	if !ok {
	...
	return p.PrepareBlock(ctx, a.canon.canonical(req))
}

func (a *node1FollowApp) FinalizeBlock(ctx context.Context, req *abci.RequestFinalizeBlock) (*abci.ResponseFinalizeBlock, error) {
	resp, err := a.Application.FinalizeBlock(ctx, a.canon.canonical(req))
	if err != nil {
		return resp, err
	}
	now := time.Now()
	a.mu.Lock()
	a.hash[req.Header.Height] = bytes.Clone(resp.AppHash)
	a.done[req.Header.Height] = now
	a.mu.Unlock()
	return resp, nil
}

// node1ValHashes holds the validator's AppHash per height.
var node1ValHashes sync.Map

// node1StubApp commits every block at once: a follower whose own execution costs nothing, so the validator's cost of
// serving it is read alone.
type node1StubApp struct {
...
func (a *node1StubApp) GetValidators() []abci.ValidatorUpdate { return a.validators }
func (a *node1StubApp) LastBlockHeight() int64               { return a.last.Load() }
func (a *node1StubApp) FinalizeBlock(ctx context.Context, req *abci.RequestFinalizeBlock) (*abci.ResponseFinalizeBlock, error) {
	res := make([]*abci.ExecTxResult, len(req.Txs))
	for i := range res {
		res[i] = &abci.ExecTxResult{Code: abci.CodeTypeOK}
	}
	a.pending = req.Header.Height
	// The validator's own AppHash for the height, as the data layer checks it against the AppQC.
	for {
		if h, ok := node1ValHashes.Load(req.Header.Height); ok {
			return &abci.ResponseFinalizeBlock{TxResults: res, AppHash: h.([]byte)}, nil
		}
...
func (a *node1StubApp) Commit(context.Context) (*abci.ResponseCommit, error) {
	a.last.Store(a.pending)
	return &abci.ResponseCommit{}, nil
}

type node1Followers struct {
	exec     *node1FollowApp
	execInner interface{ AwaitCommits() error }
	stubs    []*node1StubApp
	runs     []func(context.Context) error
	closers  []func()
	// heights each stub follower had committed when the validator's last block completed
	stubAtEnd []int64
	execAtEnd int64
	// relayPort, when set, is the port every follower dials the validator through (NODE1_FOLLOW_RELAY_PORT).
	relayPort      int
	relayConns     atomic.Int64
	relayBytesDown atomic.Int64
}

// startNode1Followers builds NODE1_FOLLOW_EXEC real executing fullnodes (0 or 1) and NODE1_FOLLOWERS stub fullnodes,
// each dialing the validator as a fullnode router does.
func startNode1Followers(t *testing.T, cfg node1Config, wl *node1Workload, genDoc *types.GenesisDoc, validators []abci.ValidatorUpdate,
	addrs map[atypes.PublicKey]p2p.GigaNodeAddr) *node1Followers {
	f := &node1Followers{}
	followAddrs := addrs
	if port := node1EnvInt("NODE1_FOLLOW_RELAY_PORT", 0); port > 0 {
		followAddrs = f.startRelay(t, port, addrs)
	}
	build := func(name string, app abci.Application, store atypes.BlockStore, dir string) {
		fcfg := &p2p.GigaRouterCommonConfig{
			DialInterval:            100 * time.Millisecond,
			ValidatorAddrs:          followAddrs,
			PersistentStateDir:      utils.Some(dir),
			App:                     proxy.New(app),
			GenDoc:                  genDoc,
			MaxInboundFullnodePeers: tmconfig.DefaultMaxInboundFullnodePeers,
		...
		ds, err := p2p.BuildDataState(fcfg, store)
		require.NoError(t, err)
		key := p2p.NodeSecretKey(ed25519.TestSecretKey(crypto.Keccak256([]byte("node1-follower-" + name))))
		r, err := p2p.NewGigaFullnodeRouter(fcfg, key, ds)
		require.NoError(t, err)
		f.runs = append(f.runs, r.Run)
	}
	if node1EnvStr("NODE1_FOLLOW_EXEC", "") == "1" {
		dir := filepath.Join(cfg.dir, "follow-exec")
		require.NoError(t, os.RemoveAll(dir))
		require.NoError(t, os.MkdirAll(dir, 0o700))
		storageCfg, err := evmonly.NewValidatorStorageConfig(dir, cfg.receipts)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		storageCfg.BlockDBConfig = &blockCfg
		manager, err := bootstrap.NewGigaStorageManager(context.Background(), storageCfg)
		require.NoError(t, err)
		encoder := evmonly.NewFlatKVChangeSetEncoder(manager.SC())
		changes, err := encoder(wl.genesis)
		require.NoError(t, err)
		require.NoError(t, manager.StateDB().CommitStateChanges(genDoc.InitialHeight-1, changes))
		inner, err := evmonlyapp.NewEVMOnlyApplication(tmconfig.AutobahnEVMOnlyChainID, validators, manager, encoder)
		require.NoError(t, err)
		f.exec = &node1FollowApp{Application: inner, canon: &node1App{cfg: cfg}, hash: map[int64][]byte{}, done: map[int64]time.Time{}}
		if ac, ok := inner.(interface{ AwaitCommits() error }); ok {
			f.execInner = ac
		}
		build("exec", f.exec, manager.BlockStore(), dir)
		f.closers = append(f.closers, func() {
			if f.execInner != nil {
				_ = f.execInner.AwaitCommits()
			}
			_ = manager.Close()
		})
	}
	for i := range node1EnvInt("NODE1_FOLLOWERS", 0) {
		dir := filepath.Join(cfg.dir, fmt.Sprintf("follow-stub-%d", i))
		require.NoError(t, os.RemoveAll(dir))
		require.NoError(t, os.MkdirAll(dir, 0o700))
		// In memory: the follower's own block writes are not the serving validator's cost.
		bs, err := blockstore.New(memblock.NewBlockDB())
		require.NoError(t, err)
		stub := &node1StubApp{validators: validators}
		f.stubs = append(f.stubs, stub)
		build(fmt.Sprintf("stub-%d", i), stub, bs, dir)
		f.closers = append(f.closers, func() { _ = bs.Close() })
	}
	return f
}

// startRelay forwards every connection on 127.0.0.1:port to the validator and returns the address book the
// followers dial it through. A delay the benchmark puts on that port so reaches the followers' connections and not
// the validator's connections to itself.
func (f *node1Followers) startRelay(t *testing.T, port int, addrs map[atypes.PublicKey]p2p.GigaNodeAddr) map[atypes.PublicKey]p2p.GigaNodeAddr {
	require.Equal(t, 1, len(addrs))
	relayed := map[atypes.PublicKey]p2p.GigaNodeAddr{}
	var target string
	for k, a := range addrs {
		target = net.JoinHostPort(a.HostPort.Hostname, strconv.Itoa(int(a.HostPort.Port)))
		a.HostPort = tcp.HostPort{Hostname: "127.0.0.1", Port: uint16(port)} //nolint:gosec // a test port
		relayed[k] = a
	}
	...
	go func() {
		for {
			...
			go func() {
				...
				delay := time.Duration(node1EnvInt("NODE1_FOLLOW_RELAY_DELAY_MS", 0)) * time.Millisecond
				go func() { node1Relay(u, c, delay); done <- struct{}{} }()
				go func() { node1Relay(node1CountingWriter{c, &f.relayBytesDown}, u, delay); done <- struct{}{} }()
				<-done
			}()
		}
	}()
...
// node1Relay copies src to dst, holding each chunk for delay after it was read (NODE1_FOLLOW_RELAY_DELAY_MS: a
// development-machine stand-in for netem, which delays the application's round trips but not TCP's own).
func node1Relay(dst io.Writer, src io.Reader, delay time.Duration) {
	if delay == 0 {
		_, _ = io.Copy(dst, src)
		return
	}
	...
	for c := range ch {
		time.Sleep(time.Until(c.at.Add(delay)))
...
// node1CountingWriter adds every byte written through it to n.
type node1CountingWriter struct {
	w io.Writer
	n *atomic.Int64
}

func (c node1CountingWriter) Write(p []byte) (int, error) {
	k, err := c.w.Write(p)
	c.n.Add(int64(k))
	return k, err
}

// markEnd records how far each follower had committed when the validator's last block completed.
func (f *node1Followers) markEnd() {
	for _, s := range f.stubs {
		f.stubAtEnd = append(f.stubAtEnd, s.last.Load())
	}
}

func (f *node1Followers) close() {
	for _, c := range f.closers {
		c()
	}
}

// report prints the FOLLOW line: the executing follower's blocks completed inside the validator's steady span, its
// height when the validator finished, and every height it executed checked against the validator's AppHash.
func (f *node1Followers) report(blocks []node1Block, from int) {
	B := len(blocks)
	lo, hi := blocks[from].done, blocks[B-1].done
	valBlocks := B - 1 - from
	wall := hi.Sub(lo).Seconds()
	execIn, execLast, checked, mismatched, missing := 0, int64(0), 0, 0, 0
	firstMismatch := int64(0)
	if f.exec != nil {
		f.exec.mu.Lock()
		byHeight := map[int64][]byte{}
		for _, b := range blocks {
			byHeight[b.height] = b.appHash
		}
		for h, d := range f.exec.done {
			if d.After(lo) && !d.After(hi) {
				execIn++
			}
			if !d.After(hi) && h > execLast {
				execLast = h
			}
		}
		for h, got := range f.exec.hash {
			want, ok := byHeight[h]
			switch {
			case !ok:
				missing++
			case !bytes.Equal(want, got):
				mismatched++
				if firstMismatch == 0 || h < firstMismatch {
					firstMismatch = h
				}
			default:
				checked++
	...
	share := 0.0
	if valBlocks > 0 {
		share = float64(execIn) / float64(valBlocks)
	}
	stubMin, stubSum := int64(-1), int64(0)
	for _, h := range f.stubAtEnd {
		if stubMin < 0 || h < stubMin {
			stubMin = h
		}
		stubSum += h
	}
	stubShare := 0.0
	if n := len(f.stubAtEnd); n > 0 {
		stubShare = float64(stubSum) / float64(n) / float64(blocks[B-1].height)
	}
	fmt.Printf("NODE1 follow exec=%v stubs=%d val_blocks=%d val_wall_s=%.6f chain_blocks_per_s=%.2f exec_blocks=%d exec_share=%.6f exec_blocks_per_s=%.2f exec_height_at_end=%d val_height_at_end=%d exec_lag_blocks=%d checked=%d mismatched=%d unmatched=%d first_mismatch=%d stub_min_height_at_end=%d stub_mean_share=%.6f\n",
		f.exec != nil, len(f.stubs), valBlocks, wall, float64(valBlocks)/wall, execIn, share, float64(execIn)/wall,
		execLast, blocks[B-1].height, blocks[B-1].height-execLast, checked, mismatched, missing, firstMismatch, stubMin, stubShare)
	if f.relayPort > 0 {
		fmt.Printf("NODE1 relay port=%d conns=%d bytes_to_followers=%d\n", f.relayPort, f.relayConns.Load(), f.relayBytesDown.Load())
	}
}
